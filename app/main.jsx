...
function App() {
  ...
  return <main className="min-h-screen px-4 py-6 text-slate-100 sm:px-8">
    <header className="mx-auto mb-6 flex max-w-7xl items-end justify-between gap-4">
      ...
      <span className="hidden rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-xs text-emerald-300 sm:block">Connected</span>
    </header>
    <section className="mx-auto grid max-w-7xl overflow-hidden rounded-3xl border border-white/10 bg-slate-900/80 shadow-2xl shadow-black/30 lg:grid-cols-[360px_1fr]">
      <aside className="border-b border-white/10 bg-slate-900/70 p-5 lg:border-b-0 lg:border-r">
        ...
        <label className="label">Model</label><select className="control" value={model} onChange={(event) => setModel(event.target.value)}>{Object.entries(groupedModels).map(([group, entries]) => <optgroup label={group} key={group}>{entries.map((entry) => <option value={entry[1]} key={entry[1]}>{entry[1]}</option>)}</optgroup>)}</select>
        <div className="mt-3 rounded-xl border border-orange-400/20 bg-orange-400/5 p-3 text-xs text-orange-100"><span className="font-semibold">{category}</span><br />{selected[2]}</div>
